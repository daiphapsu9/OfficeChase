...
public class Player : ItemCollector
{
    ...
    override public void Update()
    {
        ...
        if (level == "Level4" || level =="Tutorial")
        {
            if (Input.GetKeyDown("c") && crateno >= 1)
            {
                SoundManager.PlaySound("put");
                crateno--;
                Instantiate(placeobject, transform.position - movement * 1, Quaternion.identity);
                Vector2 hi = new Vector2(UnityEngine.Random.Range(-7f, 7f), UnityEngine.Random.Range(-3.5f, 3.5f));
                Instantiate(placeoboss, hi, Quaternion.identity);
            }
            if (Input.GetKeyDown("f") && AxeNum >= 1)
            {
